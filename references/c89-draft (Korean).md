...
**전방 참조**: 환경 절(2.)에서는 많은 전방 참조들 가운데 일부만 표시해 두었다.
...
**전방 참조**: 조건부 포함 (3.8.1), 식별자의 연결 (3.1.2.2), 소스 파일 포함 (3.8.2).
...
**전방 참조**: 어휘 요소 (3.1), 전처리 지시자 (3.8), 삼중 문자 시퀀스 (2.2.1.1).
...
**전방 참조**: 용어 정의 (4.1.1), exit 함수 (4.10.4.3).
...
* 대화형 장치의 입출력 동작 특성은 4.9.3에 규정된 대로 이루어져야 한다. 본 요구사항의 의도는 버퍼링되지 않은 출력이나 줄 단위로 버퍼링된 출력이 가능한 한 빨리 나타나게 하여, 프로그램이 입력을 기다리기 전에 프롬프트 메시지가 실제로 표시되는 것을 보장하는 것이다.
...
**전방 참조**: 복합문, 또는 블록 (3.6.2), 파일 (4.9.3), 시퀀스 포인트 (3.3, 3.6), signal 함수 (4.7), 타입 한정자 (3.5.3).
...
**전방 참조**: 문자 상수 (3.1.3.4), 전처리 지시자 (3.8), 문자열 리터럴 (3.1.4), 주석 (3.1.9).
...
* 2.2.1에서 정의된 단일 바이트 문자들은 반드시 존재해야 한다.
...
**전방 참조**: fputc 함수 (4.9.7.3), isprint 함수 (4.3.1.7).
