...
# 1. 목차(Table of Contents) 데이터
# 제공해주신 목차 전체를 문자열로 정의합니다.
toc_data = """
...
- [A. APPENDICES](#a-appendices)
  ...
  - [A.7 INDEX](#a7-index)
"""
# (실제 사용 시에는 toc_data 변수에 질문에 올려주신 전체 목차 텍스트를 넣으시면 됩니다)

def create_link_mapper(toc_text):
    """
    목차 텍스트를 파싱하여 { '섹션번호': '#앵커링크' } 형태의 딕셔너리를 생성합니다.
    """
    mapper = {}
    # 정규식 설명:
    # \[([\w\.]+) : '[' 뒤에 나오는 '숫자.숫자' 또는 'A.숫자' 형태의 섹션 번호를 그룹 1로 캡처
    # \s+.*?\]    : 공백과 섹션 이름, 닫는 대괄호 ']'
    # \((#.*?)\)  : '(' 뒤에 나오는 '#링크'를 그룹 2로 캡처
    pattern = re.compile(r'\[([\w\.]+)\s+.*?\]\((#.*?)\)')
    
    for match in pattern.finditer(toc_text):
        sec_num = match.group(1) # 예: 1.6
        link = match.group(2)    # 예: #16-definitions-of-terms
        mapper[sec_num] = link
        
    return mapper

def add_links_to_references(text, mapper):

    text = text.replace("Forward references:", "**Forward references:**")
    
    """
    텍스트 내의 (섹션번호) 패턴을 찾아 링크를 적용합니다.
    """
    # 정규식 설명:
    # \(          : 여는 괄호 '('
    # ([\w\.]+)   : 섹션 번호 (그룹 1) - 예: 1.6, A.6.3.1
    # \)          : 닫는 괄호 ')'
    ref_pattern = re.compile(r'([\w\.]+)')

    def replace_func(match):
        sec_num = match.group(1)
        # 매퍼에 해당 섹션 번호가 존재하면 링크 포맷으로 변경
        if sec_num in mapper:
            link = mapper[sec_num]
            return f"[{sec_num}]({link})"
        # 없으면 원본 유지
        return match.group(0)

    return ref_pattern.sub(replace_func, text)

# --- 실행 예시 ---

# 1. 매핑 생성 (전체 목차 텍스트가 toc_data에 있다고 가정)
section_map = create_link_mapper(toc_data)

# 2. 변환 수행
result_text = add_links_to_references(input_text, section_map)

print(f"변경 전: {input_text}")
print(f"변경 후: {result_text}")
